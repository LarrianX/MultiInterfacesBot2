...
class TelegramUser(TelegramEntity, BaseUser):
    ...
    @classmethod
    async def from_tl(cls, tl: PeerUser | ChatParticipant | ChatParticipantCreator | ChatParticipantAdmin | User, caller: TelegramInterfaceStub):
...
class TestTelegramUser(TestCase):
    def test_telegram_user_initialization(self):
        with self.client:
            user: TelegramUser = self.client.loop.run_until_complete(TelegramUser.from_tl(PeerUser(user_id=1667209703), self))
